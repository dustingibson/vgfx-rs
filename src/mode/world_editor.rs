...
impl WorldEditor {
    pub fn new(sdl_payload: &mut SDLContext, camera: &mut Camera) -> Self {
        ...
        let mut cursor_cuboid = Cuboid::from_texture(camera.position, "test".to_string(), glm::vec4(0.0,0.0,1.0,1.0), 0.05, 0.05, 0.05);
    ...
    pub fn run(&mut self, sdl_context: &mut SDLContext, camera: &mut Camera, shader: &mut ShaderContainer) {
        ...
        if(sdl_context.left_click)
        {
            ...
            let mut marker_model = Model::new(glm::vec3(0.0,0.0,0.0));
            let mut marker_cuboid = Cuboid::from_texture(final_pos, "test".to_string(), glm::vec4(0.0,0.0,1.0,1.0), size, size, size);
